---
title: "Segment Data Integration"
slug: "segment"
hidden: false
createdAt: "2023-04-26T19:44:00.382Z"
updatedAt: "2023-06-12T11:52:17.556Z"
---
The [Segment data integration](https://github.com/fingerprintjs/fingerprint-pro-segment-source-function) allows you to use Fingerprint visitor identification data as a [data source](https://segment.com/docs/connections/sources/) in Segment. It consists of an open-source [Segment Source Function](https://Segment.com/docs/connections/functions/source-functions/) which  receives the data through Fingerprint [webhooks](https://dev.fingerprint.com/docs/webhooks) and normalizes them into [Segment Specs](https://segment.com/docs/connections/spec/).

Once your Fingerprint data is inside Segment, you can use it in any of your Segment destinations to detect suspicious activity, prevent fraud or personalise user experiences.

![](https://files.readme.io/453208a-small-segment-overview.png)

> 🚧 Requirements and limitations
> 
> - You need to have the Fingerprint [JavaScript agent](https://dev.fingerprint.com/docs/js-agent) installed on your website and configured to send data to Segment via Fingerprint webhooks. Installing Segment on your website directly is not required. 
...
6. Click **Configure**, and enter a descriptive name, such as `Fingerprint Source Function`.
...
Continuing from the previous step (or if you have closed the browser tab, go to **Connections** > **Catalog** > **Functions** > **Fingerprint Source Function**)

1. Click **Connect Source**.
2. Add a descriptive name, for example `Fingerprint visitor identification events`. Optionally, you can put labels of your choice.
...
Now that the JS Agent and Segment Source are connected, the Specs should be flowing to your segment account as visitors come your website and are identified by Fingerprint. 
...
The Fingerprint Segment Source function creates Specs based on the the [visitor identification result](https://dev.fingerprint.com/docs/js-agent#get-response) and the metadata you pass to the `tag.integration.segment` object. 
...
| Identify Spec field | Source field in Fingerprint `result` or metadata passed to `segment.identify`                                                                     |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `traits.createdAt`  | [`result.firstSeenAt.subscription`](https://dev.fingerprint.com/docs/js-agent#firstseenat) - time of the first visit recorded in your application |
...
| Page Spec field           | Source field in Fingerprint `result` or metadata passed to `segment`                                                                     |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `anonymousId`             | [`result.visitorId`](https://dev.fingerprint.com/docs/js-agent#visitorid) - Fingerprint `visitorId`                                      |
| `userId`                  | `identify.userId` - your internal user ID                                                                                                |
...
| Track Spec field | Source field in Fingerprint `result` or metadata passed to `segment.track`                          |
| ---------------- | --------------------------------------------------------------------------------------------------- |
...
| Group Spec field | Source field in Fingerprint `result` or metadata passed to `segment`                                |
| ---------------- | --------------------------------------------------------------------------------------------------- |
